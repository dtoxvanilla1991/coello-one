...
function Render({
  ...
  completed,
  labels,
}: RenderProps & { labels: CountdownLabels }) {
  if (completed) {
    // Render a completed state
    return <span>{labels.completed}</span>;
  } else {
    // Render a countdown
    return (
      <div className="text-gray-1000 flex items-center gap-3 text-center text-3xl font-medium md:text-5xl xl:gap-4 xl:text-6xl 2xl:gap-6 2xl:text-7xl">
        <div className="min-w-[45px] md:min-w-[70px] xl:min-w-[90px] 2xl:min-w-[100px]">
          ...
          <p className="text-gray-1000/50 mt-2 text-xs font-semibold uppercase xl:text-sm">
            {labels.day}
          </p>
        </div>
...
export default function CountdownTimer({ labels }: CountdownTimerProps) {
  ...
  return (
    <div>
      <Countdown date={targetDate} renderer={(props) => <Render {...props} labels={labels} />} />
    </div>
  );
}
